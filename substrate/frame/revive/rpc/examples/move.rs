...
use crate::subxt_client::Error;
use crate::subxt_client::system::events;
use pallet_revive_eth_rpc::subxt_client::revive::events::Instantiated;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let selector = hex::decode(args.get(2).expect("selector not present")).expect("selector not in hex");
